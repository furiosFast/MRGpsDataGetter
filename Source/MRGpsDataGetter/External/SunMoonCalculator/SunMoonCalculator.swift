...
class SunMoonCalculator {
    ...
    static func getDate(jd: Double) throws -> Date {
        if jd.isNaN || (jd < 2_299_160 && jd >= 2_299_150) {
            throw Errors.invalidJulianDay(jd: jd)
        }
    ...
    private func getSun() -> [Double] {
        ...
        return [slongitude, slatitude, sdistance, atan(696_000 / (SunMoonCalculator.AU * sdistance))]
    }

    private func getMoon() -> [Double] {
        // MOON PARAMETERS (Formulae from "Calendrical Calculations")
        let phase: Double = SunMoonCalculator.normalizeRadians((297.8502042 + 445_267.1115168 * t - 0.00163 * t * t + t * t * t / 538_841 - t * t * t * t / 65_194_000).toRadians)
    ...
    private func doCalc(_ pos: [Double]) -> [Double] {
        ...
        let msday: Double = 1 + (((((-1.86e-5 * T0) + 0.186208) * T0) + 8_640_184.812866) / (SunMoonCalculator.SECONDS_PER_DAY * SunMoonCalculator.JULIAN_DAYS_PER_CENTURY))
        ...
        if alt > -3.toRadians {
            let r = 0.016667.toRadians * abs(tan(Double.pi / 2 - (alt.toDegrees + 7.31 / (alt.toDegrees + 4.4)).toRadians))
        ...
        if transit_alt > -3.toRadians {
            let r = 0.016667.toRadians * abs(tan(Double.pi / 2 - (transit_alt.toDegrees + 7.31 / (transit_alt.toDegrees + 4.4)).toRadians)),
                                              ...
                                              moonLon: Double, moonLat: Double, moonRA: Double, moonDec: Double) -> [Double]
    {
        ...
        let F: Double = (93.2720993 + 483_202.0175273 * t - 0.0034029 * t * t
            - t * t * t / 3_526_000 + t * t * t * t / 863_310_000).toRadians
        // Moon's inclination
        let I = 1.54242.toRadians
        // Moon's mean ascending node longitude
        let omega: Double = (125.0445550 - 1934.1361849 * t + 0.0020762 * t * t
            + t * t * t / 467_410 - t * t * t * t / 18_999_000).toRadians
        // Obliquity of ecliptic (approx, better formulae up)
        let eps = 23.43929.toRadians
    ...
    static let AU: Double = 149_597_870.691
    ...
    static let J2000: Double = 2_451_545
    ...
    private var jd_UT: Double = 0, t: Double = 0, obsLon: Double = 0, obsLat: Double = 0, TTminusUT: Double = 0,
                twilight: Twilight = .Horizon34arcmin, slongitude: Double = 0, sanomaly: Double = 0
